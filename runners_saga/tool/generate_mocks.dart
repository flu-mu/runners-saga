...
// Run with: dart run tool/generate_mocks.dart
import 'dart:io';
...
Future<void> _episodeDetails(Directory dir) async {
  final canvas = img.Image(width: 1170, height: 2532); // 3x iPhone 13
  // Gradient header
  for (int y = 0; y < 660; y++) {
    final t = y / 660.0;
...
Future<void> _runScreen(Directory dir) async {
  final canvas = img.Image(width: 1170, height: 2532);
...
img.Image _sheetBase(String title) {
  final canvas = img.Image(width: 1170, height: 1600);
