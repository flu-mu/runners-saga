...
class _RunScreenState extends ConsumerState<RunScreen> with WidgetsBindingObserver {
  ...
  DateTime? _pauseStartTime; // ADDED: New variable to record the pause start time
  ...
  StepDetectionService? _stepService; // for step-counting mode
  ...
  void _onAppResumed() {
    ...
    final resumeMode = ref.read(trackingModeProvider) ?? TrackingMode.gps;
    final resumeTrackingEnabled = ref.read(trackingEnabledProvider);
    if (_isTimerRunning && !_isPaused && _gpsSubscription == null && resumeMode == TrackingMode.gps && resumeTrackingEnabled) {
      _startGpsTracking();
    }
    // Resume step detection if needed
    final mode = ref.read(trackingModeProvider) ?? TrackingMode.gps;
    final trackingEnabled = ref.read(trackingEnabledProvider);
    if (mode == TrackingMode.steps && trackingEnabled && _isTimerRunning && !_isPaused) {
      _startStepDetection();
    }
  ...
  void _calculateCurrentPace() {
    ...
    _recentGpsPoints = _gpsRoute.where((position) =>
      // FIX: Changed 'point.createdAt' to 'position.timestamp'
  ...
  void _onGpsPositionUpdate(Position position) {
    ...
    // FIX: Removed the incorrect call to addDistance on progressMonitor
    // runSessionManager.state.progressMonitor.addDistance(distance);
  ...
  void _startSimpleTimer() async {
    ...
    // Start step detection if using step mode
    try {
      final mode = ref.read(trackingModeProvider) ?? TrackingMode.gps;
      final trackingEnabled = ref.read(trackingEnabledProvider);
      if (mode == TrackingMode.steps && trackingEnabled) {
        _startStepDetection();
      }
    } catch (_) {}
  ...
  void _startStepDetection() {
    _stepService ??= StepDetectionService();
    _stepService!.start((delta) {
      try {
        final manager = ref.read(runSessionControllerProvider.notifier).state;
        manager.progressMonitor.addSteps(delta);
      } catch (e) {
        // ignore
      }
    });
  ...
  /// Pause the simple timer (also flips UI state even if local timer isn't running)
  void _pauseSimpleTimer() async {
    if (_isPaused) return;
    
    _isPaused = true;
    _pauseStartTime = DateTime.now(); // ADDED: Record the exact time of the pause
    
    // Cancel local timer if present
    if (_simpleTimer != null) {
      _simpleTimer!.cancel();
      _simpleTimer = null;
    }
    
    // Pause audio and background session
    try {
  ...
  void _stopSimpleTimer() {
    ...
    _totalPausedTime = Duration.zero; // ADDED: Reset total paused time on stop
    ...
    try {
      ...
      if ((targetTime != null || targetDistance != null) && !_timerStopped) {
        ...
        if (canStart) {
          ...
          // FIX: Removed the incorrect 'onRouteUpdated' setter.
          // The RunMapPanel should listen to changes via the provider, not
          // through a manual callback. The setState() call in onGpsPositionUpdate
          // already handles UI updates.
  ...
  void _resumeTimer() async {
    ...
    // ADDED: Calculate the duration of the pause and add it to the total paused time
    if (_pauseStartTime != null) {
      _totalPausedTime += DateTime.now().difference(_pauseStartTime!);
      _pauseStartTime = null; // Reset the pause start time
    }
    ...
    _isPaused = false;
    // Only mark as running and restart local timer if we actually want to run it
    _isTimerRunning = true;
